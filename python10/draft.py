  ...
  # INSERTING DATA TO TABLE FROM CSV FILE
    with connection.cursor() as cursor:
        ...
        """
            ...
            FROM 'C:/Users/User/PycharmProjects/pythonProject2/python10/csv_for_lab10_phone_book.csv'
            DELIMITER ','
            CSV HEADER;
        """
        )
...
#       fetches users with phone numbers containing '23'
    cursor.execute(
        """
        SELECT user_id, user_name from phone_book WHERE phone_num like '%23%'
        """
    )
    ...
    cursor.execute(
        """
        SELECT user_name, phone_num from phone_book WHERE user_id <= 4
        """
    )
